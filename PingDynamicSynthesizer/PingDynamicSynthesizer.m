...
SEL _cmd){  \
return  objc_getAssociatedObject(_self, _ping_get_associated_objectKey(_cmd));  \
}
...
#define PingPointNotVoid(attr) ((attr[1] == '^') && (attr[2] != 'v'))
...
static inline char * _ping_set_method_encode(char *code){
    ...
    if (code[0] == '^' && code[1] == 'v') {
        char newencode[6] = "v@:^v\0";
        set_methodencode = newencode;
    }else{
        char newencode[5] = "v@:*\0";
        newencode[3] = code[0];
        set_methodencode = newencode;
    }
...
static inline char * _ping_get_method_encode(char *code){
    ...
    if (code[0] == '^' && code[1] == 'v') {
        char new_ncode[5] = "^v@:\0";
        get_methodencode = new_ncode;
    }else{
        char newencode[4] = "v@:\0";
        newencode[0] = code[0];
...
static inline uintptr_t _ping_analyze_policy(NSString *attr){
    ...
    if ([[attr substringFromIndex:(attrLen - 1)] isEqualToString:PING_NONATOMIC_ENCODE]) {
        ...
        if ([[attr substringFromIndex:attrLen - 1] isEqualToString:PING_STRONG_ENCODE]) {
            policy = OBJC_ASSOCIATION_RETAIN;
        }else if ([[attr substringFromIndex:attrLen - 1] isEqualToString:PING_COPY_ENCODE]){
            policy = OBJC_ASSOCIATION_COPY;
        }else if ([[attr substringFromIndex:attrLen - 1] isEqualToString:PING_WEAK_ENCODE]){
 ...
 */
static inline void * _ping_get_associated_objectKey(SEL sel){
    NSString *selName = NSStringFromSelector(sel);
    if (![selName hasPrefix:@"set"]) {
        return (void *)NSSelectorFromString(selName);
    }
    selName = [selName substringFromIndex:3];
    NSString  *prefixStr = [selName substringToIndex:1];
    prefixStr = [prefixStr lowercaseString];
    NSString *suffixStr = [selName substringFromIndex:1];
    suffixStr = [suffixStr stringByReplacingOccurrencesOfString:@":" withString:@""];
    selName = [NSString stringWithFormat:@"%@%@",prefixStr,suffixStr];
    return (void *)NSSelectorFromString(selName);
}
...
static void _ping_dispense_setget_implementation(uintptr_t policy,SEL setSel,SEL getSel, __nonnull Class class,char *encode){
